...
self_hosted::pack() {
  ...
  cd ~/$SEMAPHORE_GIT_DIR/cache-cli && make build OS=linux && mv bin/cache /tmp/self-hosted-Linux/toolbox && cd - > /dev/null
  cd ~/$SEMAPHORE_GIT_DIR/cache-cli && make build OS=darwin && mv bin/cache /tmp/self-hosted-Darwin/toolbox && cd - > /dev/null
}
