...
func TestJUnitPHPUnit(t *testing.T) {
	...
	t.Run("Golden File Test", func(t *testing.T) {
		test := GoldenTest{
			Name:       "JUnitPHPUnit",
			Parser:     NewJUnitPHPUnit(),
			InputFile:  FixturePath("priv/parsers/junit_phpunit/in.xml"),
			GoldenFile: FixturePath("priv/parsers/junit_phpunit/out.json"),
		}
	...
	t.Run("Parser Identification", func(t *testing.T) {
		...
		// PHPUnit parser currently returns false for all IsApplicable checks
		// It needs to be explicitly selected with -p phpunit
		AssertParserApplicable(t, p, FixturePath("priv/parsers/junit_phpunit/in.xml"), false)
